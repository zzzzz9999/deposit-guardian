...
def calculate_depreciation_rate(age_years: float, lifespan_years: float) -> float:
    """计算折旧后剩余价值比例（残值率）"""
    if lifespan_years <= 0:
        return 0.0
    rate = 1.0 - (age_years / lifespan_years)
    return max(0.05, min(1.0, rate))  # 最低保留 5% 残值
...
def calculate_deposit(
    ...
    deductions 每项格式：
    {
        ...
        item_type: str (可选，用于自动查找年限),
        item_age_years: float (可选),
        item_lifespan_years: float (可选),
        is_natural_wear: bool (可选，用户标注是否自然损耗)
    }
    """
    ...
    for d in deductions:
        ...
        lifespan = float(d.get("item_lifespan_years") or ITEM_LIFESPAN.get(d.get("item_type", ""), ITEM_LIFESPAN["default"]))

        if is_natural:
            valid = 0.0
            explanation = f"用户标注为自然损耗，依据《民法典》第713条，出租人不得要求赔偿。"
        else:
            dep_rate = calculate_depreciation_rate(age, lifespan)
        ...
        details.append({
            ...
            "depreciation_rate": round(1 - valid / claimed, 4) if claimed > 0 else 0,
    ...
    return {
        ...
        "legal_note": "以上计算基于《民法典》第713条自然损耗原则及折旧计算方法，仅供参考。具体金额以法院判决为准。"
    }
